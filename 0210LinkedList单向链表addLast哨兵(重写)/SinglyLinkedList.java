...
public class SinglyLinkedList implements Iterable<Integer> { // 整体
    ...
    // 一个node 包括 value 和 指针叫next 

    private Node head = new Node(666, null); // 头 指针 指向一个哨兵, 这样就一定有一个 Node, 没有空的情况了 
    ...
    private final class NodeIterator implements Iterator<Integer> {
        Node p = head.next; // 初始值 指向头节点 
        // 有了哨兵之后, 得从后面的地方开始遍历 所以 head 改为 head.next 
    ...
    public void loop1(Consumer<Integer> consumer) {
        Node p = head.next; // 有哨兵, next 开始遍历
        while (p != null) {
    ...
    public void loop2(Consumer<Integer> consumer) { // 有哨兵, next 开始遍历
    ...
    // 比较关键的是找上一个节点 prev 
    // 新节点 和 prev.next 都指向下一个节点 
    // 更改 prev.next 指向去 新的节点 

    // 有哨兵存在的话, findNode 能找到 index = 0, 就是哨兵 
